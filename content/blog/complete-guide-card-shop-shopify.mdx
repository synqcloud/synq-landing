...
faq:
  ...
  - question: "How do I price my trading cards on Shopify?"
    answer: "The most efficient way is to use automated pricing tools like Synq that pull market data from TCGplayer, then apply your chosen pricing strategy (market price, market plus a percentage, or market minus a percentage) to set competitive prices for the products Synq creates."
...
This is the core problem that Synq solves for the products it manages. By pulling market data from TCGplayer and writing it to your Shopify product prices, your store stays competitive without requiring you to manually update every listing. You set your pricing strategy once (market price, market plus a percentage, or market minus a percentage), and Synq applies it when prices are synced so you stay aligned with the market.
...
The best approach often combines market-based and margin-based thinking: price to market, but protect yourself with a floor so you never sell below a level you are comfortable with. Synq supports a store-wide price floor (and a ceiling) as a global price rule, so market-driven prices stay inside the range you set.
...
Synq connects your Shopify store to TCGplayer market data and keeps the prices of the products it manages current. When a card spikes in value after a tournament win, your price updates on the next sync. When a reprint tanks a previously expensive card, your price adjusts downward so you stay competitive and keep selling.

You stay in control by setting your pricing strategy (market price, market plus a percentage, or market minus a percentage) along with store-wide condition adjustments and optional global price rules for a floor, ceiling, and rounding. Automatic repricing runs once a day at an hour you choose in your store timezone, on the Pro and Scale plans, and a one-click "Sync all prices" button on the Products page lets you reprice on demand on any plan. The sync writes the variant price only, so your stock, titles, descriptions, and images are left untouched. This means you can price thousands of cards accurately without touching a spreadsheet.
...
### Staying in Control of Price Changes

Automation should not mean flying blind. With Synq, the products you have added live on the Products page, where you can run a manual "Sync all prices" and review the proposed changes before they go live. That keeps you in the loop on how market movements affect your catalog so you can spot cards that are trending upward and decide when to reprice, rather than handing pricing over to a black box.
...
Have a clear return policy posted on your store. Most card shops accept returns within 30 days if the card does not match the listed condition. Be fair and responsive, since a quick resolution to a legitimate complaint costs far less than a chargeback or a negative review that scares away future customers.
...
Card prices move fast. If you are updating prices once a week, or worse, once a month, you are guaranteed to be overpriced on some cards (losing sales) and underpriced on others (losing margin). Automate your pricing updates so they happen daily, the way Synq's automatic daily repricing does on the Pro and Scale plans.
